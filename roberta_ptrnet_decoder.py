...
from transformers import RobertaTokenizerFast, RobertaModel, RobertaTokenizer
...
def build_vocab(tr_data, dv_data, save_vocab, embedding_file):
    vocab = OrderedDict()
    char_v = OrderedDict()
    char_v['<PAD>'] = 0
    char_v['<UNK>'] = 1
    char_idx = 2
    for d in tr_data + dv_data:
...
def get_pred_triples_for_relation(rel, arg1s, arg1e, arg2s, arg2e, src_words, t_rel):
    '''Converts the predicted output to triples'''
    triples = [] #contains only the unique triples, i.e. no duplicates
    all_triples = [] #contains all triples, including the duplicates
    for i in range(0, len(rel)):
        ...
        #if the triple doesnt correspond to the passed t_rel relation, then ignore it
        if(relIdxToName[r]!=t_rel):
            continue
...
def get_gt_triples_for_relation(src_words, rels, pointers, t_rel):
    '''Returns a list of triples generated using words in the sentence(src_words), relations(rels) and pointers'''
    triples = []
    i = 0
    for r in rels:
        ...
        #if the triple doesnt belong to the passed relation, continue
        if(relIdxToName[r]!=t_rel):
            continue
...
def write_test_res(src, data, preds, outfile):
    reader = open(src)
...
def get_F1_for_relation(data, preds, t_rel):
    '''Returns the F1 for only the relation (t_rel)'''
    gt_pos = 0
    pred_pos = 0
    total_pred_pos = 0
    correct_pos = 0
    for i in range(0, len(data)):
        gt_triples = get_gt_triples_for_relation(data[i].SrcWords, data[i].TrgRels, data[i].TrgPointers, t_rel)

        pred_triples, all_pred_triples = get_pred_triples_for_relation(preds[0][i], preds[1][i], preds[2][i], preds[3][i],
                                                          preds[4][i], data[i].SrcWords, t_rel)
        total_pred_pos += len(all_pred_triples)
        gt_pos += len(gt_triples)
        pred_pos += len(pred_triples)
        for gt_triple in gt_triples:
...
def get_F1_for_entity_set(data, preds, entity_set):
    '''Returns values used to compute scores for the set of entities present'''
    gt_pos = 0
    pred_pos = 0
    total_pred_pos = 0
    correct_pos = 0
    for i in range(0, len(data)):
        gt_triples = get_gt_triples_for_entity_set(data[i].SrcWords, data[i].TrgRels, data[i].TrgPointers, entity_set)

        pred_triples, all_pred_triples = get_pred_triples_for_entity_set(preds[0][i], preds[1][i], preds[2][i], preds[3][i],
                                                          preds[4][i], data[i].SrcWords, entity_set)
        total_pred_pos += len(all_pred_triples)
        gt_pos += len(gt_triples)
        pred_pos += len(pred_triples)
        for gt_triple in gt_triples:
...
def train_model(model_id, train_samples, dev_samples, best_model_file):
    ...
    df = pd.DataFrame(columns = ['epoch number','training loss'])
    ...
    for epoch_idx in range(0, num_epoch):
        ...
        df.loc[0] = epoch_idx + 1
        df.loc[1] = train_loss_val
...
if __name__ == "__main__":
    ...
    if not os.path.exists('/content/drive/My Drive/New_dataset/RoBERTa_output'):
        os.mkdir('/content/drive/My Drive/New_dataset/RoBERTa_output')
    model_name = 1
    job_mode = sys.argv[3]
    ...
    gen_direct = gen_directions[int(sys.argv[6])]
    triplet_orders = ['Random', 'AP_OP', 'OP_AP']
    trip_order = triplet_orders[int(sys.argv[7])]

    bert_base_size = 768
    update_bert = bool(int(sys.argv[8]))
    bert_model_name = "roberta-base"
    bert_tokenizer = RobertaTokenizer.from_pretrained(bert_model_name)
    #print(len(bert_tokenizer(text, truncation=True).input_ids))
    ...
    embedding_file = '/content/drive/My Drive/New_dataset/w2v.txt'
    update_freq = 1
    wf = 1.0
    att_type = 2
    max_dist = 10
    ...
    rel_file = os.path.join('/content/drive/My Drive/New_dataset/relations.txt')
    relnameToIdx, relIdxToName = get_relations(rel_file)

    # train a model
    if job_mode == 'train':
        logger = open(os.path.join('/content/drive/My Drive/New_dataset/RoBERTa_output/training.log'), 'w')
        ...
        model_file_name = os.path.join('/content/drive/My Drive/New_dataset/RoBERTa_output/model.h5py')

        src_train_file = os.path.join('/content/drive/My Drive/New_dataset/train_bert.sent')
        trg_train_file = os.path.join('/content/drive/My Drive/New_dataset/train_bert.pointer')
        train_pos_file = os.path.join('/content/drive/My Drive/New_dataset/train_bert.pos')
        train_data = read_data(src_train_file, trg_train_file, train_pos_file, 1)

        # train_data = train_data[:100]

        src_dev_file = os.path.join('/content/drive/My Drive/New_dataset/dev_bert.sent')
        trg_dev_file = os.path.join('/content/drive/My Drive/New_dataset/dev_bert.pointer')
        dev_pos_file = os.path.join('/content/drive/My Drive/New_dataset/dev_bert.pos')
        ...
        #src_test_file = os.path.join('/content/drive/My Drive/BERT_Tapas/test_bert.sent')
        #trg_test_file = os.path.join('/content/drive/My Drive/BERT_Tapas/test_bert.pointer')
        test_pos_file = os.path.join('/content/drive/My Drive/New_dataset/test_bert.pos')
        ...
        save_vocab = os.path.join('/content/drive/My Drive/New_dataset/RoBERTa_output/vocab.pkl')

        custom_print("getting pos tags......")
        pos_vocab = build_tags(train_pos_file, dev_pos_file, test_pos_file)

        word_vocab, char_vocab, word_embed_matrix = build_vocab(train_data, dev_data,
    ...
    if job_mode == 'test':
        logger = open(os.path.join('/content/drive/My Drive/New_dataset/RoBERTa_output/test.log'), 'w')
        custom_print(sys.argv)
        custom_print("loading word vectors......")
        vocab_file_name = os.path.join('/content/drive/My Drive/New_dataset/RoBERTa_output/vocab.pkl')
        ...
        model_file = os.path.join('/content/drive/My Drive/New_dataset/RoBERTa_output/model.h5py')
        ...
        custom_print('\nTest Results\n')
        src_test_file = os.path.join('/content/drive/My Drive/New_dataset/test_bert.sent')
        trg_test_file = os.path.join('/content/drive/My Drive/New_dataset/test_bert.pointer')
        test_pos_file = os.path.join('/content/drive/My Drive/New_dataset/test_bert.pos')
        # adj_test_file = os.path.join(src_data_folder, 'test.dep')
        test_data = read_data(src_test_file, trg_test_file, test_pos_file, 3)
        custom_print('Test data size:', len(test_data))

        #reader = open(os.path.join('/content/drive/My Drive/BERT_Tapas/test.tup'))
        #test_gt_lines = reader.readlines()
        #reader.close()
        test_preds = predict(test_data, best_model, model_name)

        t_rels = ['Voltage', 'Capacity', 'Conductivity', 'Coulombic_Efficiency', 'Energy']
        for t_rel in t_rels:
          
            pred_pos, gt_pos, correct_pos = get_F1_for_relation(test_data, test_preds, t_rel)
            custom_print(pred_pos, '\t', gt_pos, '\t', correct_pos)
            p = float(correct_pos) / (pred_pos + 1e-8)
            r = float(correct_pos) / (gt_pos + 1e-8)
            test_f1 = (2 * p * r) / (p + r + 1e-8)
            print('For relation : ' + str(t_rel))
            custom_print('P:', round(p, 3))
            custom_print('R:', round(r, 3))
            custom_print('F1:', round(test_f1, 3))
            print("\n")
        ...
        write_test_res(src_test_file, test_data, test_preds,
                       os.path.join('/content/drive/My Drive/New_dataset/RoBERTa_output/test.out'))
